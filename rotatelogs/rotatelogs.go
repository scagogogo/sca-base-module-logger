...
func New(p string, options ...Option) (*RotateLogs, error) {
	...
	// 2022-4-15 12:56:21 为了能够捕获到所有日志
	globPattern = filepath.Dir(p) + "/*.log"
	...
	if forceNewFile {
		...
		for {
			if generation == 0 {
				...
				// 2022-4-15 12:59:36 这里补零让其能够对齐，后面删除的时候按照字典序去删除，就能保留最近的N个文件了
				name = fmt.Sprintf("%s-%03d.log", strings.ReplaceAll(filename, ".log", ""), generation)
			}
...
func (rl *RotateLogs) rotate_nolock(filename string) error {
	...
	if rl.rotationCount > 0 {
		...
		// 2022-4-15 14:54:54 不再按字典序去删除，而是使用自定义的删除策略
		//toUnlink = toUnlink[:len(toUnlink)-int(rl.rotationCount)]
		toUnlink = selectNeedDeleteLogFile(toUnlink, len(toUnlink)-int(rl.rotationCount))
	}
